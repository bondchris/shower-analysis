...
export function checkDoorBlocking(rawScan: RawScan): boolean {
  ...
  for (const door of rawScan.doors) {
    ...
    for (const objRes of objPolys) {
      ...
      // 3. Low Profile (Step-over): objMaxY < doorMinY + STEP_OVER_HEIGHT
      //    (This covers #2 as well if STEP_OVER_HEIGHT >= 0)
      if (objRes.minY > doorMaxY || objRes.maxY < doorMinY + STEP_OVER_HEIGHT) {
        continue;
      }
