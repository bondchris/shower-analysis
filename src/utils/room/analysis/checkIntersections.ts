...
/**
 * Internal helper to check for intersections between embedded objects (windows, doors, openings).
 * Uses polygon intersection checks similar to object-object intersection logic.
 */
function checkEmbeddedObjectIntersectionsInternal(rawScan: RawScan): boolean {
  const TOLERANCE = 0.0254; // 1 inch
  const DIM_X = 0;
  const DIM_Z = 2;
  const HALF = 2;
  ...
  const ZERO = 0;
  const DIM_SIZE = 3;
  const TRANSFORM_SIZE_CHECK = 16;
  const MIN_ITEMS = 0;

  // Collect all embedded objects (windows, doors, openings) with their polygons
  interface EmbeddedObjectPoly {
    innerCorners: Point[];
    story: number;
  }

  const embeddedPolys: EmbeddedObjectPoly[] = [];

  // Process windows
  for (const win of rawScan.windows) {
    if (win.transform.length !== TRANSFORM_SIZE_CHECK || win.dimensions.length !== DIM_SIZE) {
      continue;
    }

    const halfW = (win.dimensions[DIM_X] ?? DEFAULT_DIM) / HALF;
    const halfD = (win.dimensions[DIM_Z] ?? DEFAULT_DIM) / HALF;
    ...
    embeddedPolys.push({
      innerCorners: innerCornersLocal.map((c) => transformPoint(c, win.transform)),
      story: win.story
    });
  }
  ...
  for (const door of rawScan.doors) {
    if (door.transform.length !== TRANSFORM_SIZE_CHECK || door.dimensions.length !== DIM_SIZE) {
      continue;
    }

    const halfW = (door.dimensions[DIM_X] ?? DEFAULT_DIM) / HALF;
    const halfD = (door.dimensions[DIM_Z] ?? DEFAULT_DIM) / HALF;
    ...
    embeddedPolys.push({
      innerCorners: innerCornersLocal.map((c) => transformPoint(c, door.transform)),
      story: door.story
    });
  }
  ...
  for (const opening of rawScan.openings) {
    if (
      opening.transform === undefined ||
      opening.dimensions === undefined ||
      opening.transform.length !== TRANSFORM_SIZE_CHECK ||
      opening.dimensions.length !== DIM_SIZE
    ) {
      continue;
    }

    const halfW = (opening.dimensions[DIM_X] ?? DEFAULT_DIM) / HALF;
    const halfD = (opening.dimensions[DIM_Z] ?? DEFAULT_DIM) / HALF;
    ...
    const openingStory = opening.story ?? ZERO;
    embeddedPolys.push({
      innerCorners: innerCornersLocal.map((c) => transformPoint(c, opening.transform ?? [])),
      story: openingStory
    });
  }

  // Check for intersections between embedded objects
  const NEXT_IDX = 1;
  for (let i = 0; i < embeddedPolys.length; i++) {
    const polyA = embeddedPolys[i];
    if (!polyA || polyA.innerCorners.length === MIN_ITEMS) {
      continue;
    }

    for (let j = i + NEXT_IDX; j < embeddedPolys.length; j++) {
      const polyB = embeddedPolys[j];
      if (!polyB || polyB.innerCorners.length === MIN_ITEMS) {
        continue;
      }

      // Check story
      if (polyA.story !== polyB.story) {
        continue;
      }

      // Check polygon intersection
      if (doPolygonsIntersect(polyA.innerCorners, polyB.innerCorners)) {
        return true;
      }
    }
  }
...
export function checkIntersections(rawScan: RawScan): {
  ...
  const embeddedRes = checkEmbeddedObjectIntersectionsInternal(rawScan);
  return {
    hasEmbeddedObjectIntersectionErrors: embeddedRes,
