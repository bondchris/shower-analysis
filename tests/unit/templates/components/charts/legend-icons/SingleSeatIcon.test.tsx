...
import { SingleSeatIcon } from "../../../../../../src/templates/components/charts/legend-icons/iconConfig";
...
describe("SingleSeatIcon", () => {
  ...
  it("should render with correct transform and scale", () => {
    ...
    const { container } = renderIconInSvg(
      <SingleSeatIcon color={TEST_COLOR} x={TEST_X} y={TEST_Y} legendBoxSize={TEST_LEGEND_BOX_SIZE} />
    );

    const gElement = container.querySelector("g");
    expect(gElement).not.toBeNull();
    const expectedScale = TEST_LEGEND_BOX_SIZE / SINGLE_SEAT_VIEWBOX_WIDTH;
  ...
  it("should call loadSvgContent with correct path and color", () => {
    ...
    renderIconInSvg(<SingleSeatIcon color={TEST_COLOR} x={TEST_X} y={TEST_Y} legendBoxSize={TEST_LEGEND_BOX_SIZE} />);

    expect(loadSvgContentSpy).toHaveBeenCalledWith("src/templates/assets/icons/single-seat-sofa.svg", TEST_COLOR);
  });
  ...
  it("should scale correctly for different legendBoxSize values", () => {
    ...
    const { container } = renderIconInSvg(
      <SingleSeatIcon color={TEST_COLOR} x={TEST_X} y={TEST_Y} legendBoxSize={largeBoxSize} />
    );

    const gElement = container.querySelector("g");
    const expectedScale = largeBoxSize / SINGLE_SEAT_VIEWBOX_WIDTH;
  ...
  it("should handle different color values", () => {
    ...
    const { container } = renderIconInSvg(
      <SingleSeatIcon color={differentColor} x={TEST_X} y={TEST_Y} legendBoxSize={TEST_LEGEND_BOX_SIZE} />
    );
  ...
  it("should handle different x and y positions", () => {
    ...
    const customY = 100;
    const { container } = renderIconInSvg(
      <SingleSeatIcon color={TEST_COLOR} x={customX} y={customY} legendBoxSize={TEST_LEGEND_BOX_SIZE} />
    );
