...
import { ChairArmExistingIcon } from "../../../../../../src/templates/components/charts/legend-icons/iconConfig";
...
describe("ChairArmExistingIcon", () => {
  const TEST_COLOR = "#ff0000";
  const TEST_X = 10;
  const TEST_Y = 20;
  const TEST_LEGEND_BOX_SIZE = 12;
  const MOCK_SVG_CONTENT = '<path fill="#ff0000" d="M134.342 320H346.032"/>';
  const CHAIR_ARM_EXISTING_VIEWBOX_WIDTH = 512;

  it("should render with correct transform and scale", () => {
    vi.spyOn(svgLoader, "loadSvgContent").mockReturnValue(MOCK_SVG_CONTENT);

    const { container } = render(
      <ChairArmExistingIcon color={TEST_COLOR} x={TEST_X} y={TEST_Y} legendBoxSize={TEST_LEGEND_BOX_SIZE} />
    );

    const gElement = container.querySelector("g");
    expect(gElement).not.toBeNull();
    const expectedScale = TEST_LEGEND_BOX_SIZE / CHAIR_ARM_EXISTING_VIEWBOX_WIDTH;
  ...
  it("should call loadSvgContent with correct path and color", () => {
    ...
    render(<ChairArmExistingIcon color={TEST_COLOR} x={TEST_X} y={TEST_Y} legendBoxSize={TEST_LEGEND_BOX_SIZE} />);

    expect(loadSvgContentSpy).toHaveBeenCalledWith("src/templates/assets/icons/chair-arm-existing.svg", TEST_COLOR);
  });
  ...
  it("should scale correctly for different legendBoxSize values", () => {
    ...
    const { container } = render(
      <ChairArmExistingIcon color={TEST_COLOR} x={TEST_X} y={TEST_Y} legendBoxSize={largeBoxSize} />
    );

    const gElement = container.querySelector("g");
    const expectedScale = largeBoxSize / CHAIR_ARM_EXISTING_VIEWBOX_WIDTH;
  ...
  it("should handle different color values", () => {
    ...
    vi.spyOn(svgLoader, "loadSvgContent").mockReturnValue(`<path fill="${differentColor}" d="M134.342 320H346.032"/>`);

    const { container } = render(
      <ChairArmExistingIcon color={differentColor} x={TEST_X} y={TEST_Y} legendBoxSize={TEST_LEGEND_BOX_SIZE} />
    );
  ...
  it("should handle different x and y positions", () => {
    ...
    const customY = 100;
    const { container } = render(
      <ChairArmExistingIcon color={TEST_COLOR} x={customX} y={customY} legendBoxSize={TEST_LEGEND_BOX_SIZE} />
    );
