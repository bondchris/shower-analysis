...
import { DoorOpenIcon } from "../../../../../../src/templates/components/charts/legend-icons/iconConfig";
...
describe("DoorOpenIcon", () => {
  const TEST_COLOR = "#ff0000";
  const TEST_X = 10;
  const TEST_Y = 20;
  const TEST_LEGEND_BOX_SIZE = 12;
  const MOCK_SVG_CONTENT = '<path fill="#ff0000" d="M0,115.27"/>';
  const OPEN_DOOR_VIEWBOX_WIDTH = 122.88;
  ...
  it("should render with correct transform and scale", () => {
    ...
    const { container } = renderIconInSvg(
      <DoorOpenIcon color={TEST_COLOR} x={TEST_X} y={TEST_Y} legendBoxSize={TEST_LEGEND_BOX_SIZE} />
    );

    const gElement = container.querySelector("g");
    expect(gElement).not.toBeNull();
    const expectedScale = TEST_LEGEND_BOX_SIZE / OPEN_DOOR_VIEWBOX_WIDTH;
  ...
  it("should call loadSvgContent with correct path and color", () => {
    ...
    renderIconInSvg(<DoorOpenIcon color={TEST_COLOR} x={TEST_X} y={TEST_Y} legendBoxSize={TEST_LEGEND_BOX_SIZE} />);

    expect(loadSvgContentSpy).toHaveBeenCalledWith("src/templates/assets/icons/door-open.svg", TEST_COLOR);
  });
  ...
  it("should scale correctly for different legendBoxSize values", () => {
    ...
    const { container } = renderIconInSvg(
      <DoorOpenIcon color={TEST_COLOR} x={TEST_X} y={TEST_Y} legendBoxSize={largeBoxSize} />
    );

    const gElement = container.querySelector("g");
    const expectedScale = largeBoxSize / OPEN_DOOR_VIEWBOX_WIDTH;
  ...
  it("should handle different color values", () => {
    ...
    vi.spyOn(svgLoader, "loadSvgContent").mockReturnValue(`<path fill="${differentColor}" d="M0,115.27"/>`);

    const { container } = renderIconInSvg(
      <DoorOpenIcon color={differentColor} x={TEST_X} y={TEST_Y} legendBoxSize={TEST_LEGEND_BOX_SIZE} />
    );
  ...
  it("should handle different x and y positions", () => {
    ...
    const customY = 100;
    const { container } = renderIconInSvg(
      <DoorOpenIcon color={TEST_COLOR} x={customX} y={customY} legendBoxSize={TEST_LEGEND_BOX_SIZE} />
    );
