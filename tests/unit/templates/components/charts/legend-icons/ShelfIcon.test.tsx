...
import { ShelfIcon } from "../../../../../../src/templates/components/charts/legend-icons/iconConfig";
...
describe("ShelfIcon", () => {
  ...
  it("should call loadSvgContent with correct path and color", () => {
    ...
    render(<ShelfIcon color={TEST_COLOR} x={TEST_X} y={TEST_Y} legendBoxSize={TEST_LEGEND_BOX_SIZE} />);

    expect(loadSvgContentSpy).toHaveBeenCalledWith("src/templates/assets/icons/shelf.svg", TEST_COLOR);
  });
  ...
  it("should scale correctly for different legendBoxSize values", () => {
    ...
    const { container } = render(<ShelfIcon color={TEST_COLOR} x={TEST_X} y={TEST_Y} legendBoxSize={largeBoxSize} />);

    const gElement = container.querySelector("g");
    const expectedScale = largeBoxSize / SHELF_VIEWBOX_WIDTH;
  ...
  it("should handle different color values", () => {
    ...
    const { container } = render(
      <ShelfIcon color={differentColor} x={TEST_X} y={TEST_Y} legendBoxSize={TEST_LEGEND_BOX_SIZE} />
    );
  ...
  it("should handle different x and y positions", () => {
    ...
    const customY = 100;
    const { container } = render(
      <ShelfIcon color={TEST_COLOR} x={customX} y={customY} legendBoxSize={TEST_LEGEND_BOX_SIZE} />
    );
