...
describe("chartUtils", () => {
    ...
    describe("Pure Functions (Logic)", () => {
        describe("calculateHistogramBins", () => {
            const DEFAULT_OPTIONS = { binSize: 2, min: 0, max: 10 };

            it("should bucket data correctly", () => {
                const data = [-1, 0, 1.9, 2, 9.999, 10, 100];
                const { buckets, labels } = calculateHistogramBins(data, DEFAULT_OPTIONS);
            ...
            it("should throw on invalid inputs", () => {
                expect(() => calculateHistogramBins([], { binSize: 0, min: 0, max: 10 })).toThrow(/Invalid binSize/);
                expect(() => calculateHistogramBins([], { binSize: 1, min: 10, max: 0 })).toThrow(/Invalid range/);
                expect(() => calculateHistogramBins([], { binSize: 0.0000001, min: 0, max: 10000 })).toThrow(/safety limit/);
            });
        });

        describe("calculateHistogramBinCenter", () => {
            const OPS = { min: 0, max: 10, binSize: 2 };
    ...
    describe("Config Builders", () => {
        it("buildLineChartConfig validates inputs", () => {
            expect(() => buildLineChartConfig(["A"], [{ label: "L", data: [], borderColor: "r" }]))
                .toThrow(/mismatch/);
        });

        it("buildLineChartConfig creates correct structure", () => {
            const config = buildLineChartConfig(["A"], [{ label: "L", data: [1], borderColor: "r" }]);
    ...
    describe("Integration (create functions)", () => {
        ...
        it("createLineChart calls render", async () => {
            await createLineChart(["A"], [{ label: "L", data: [1], borderColor: "red" }]);
            expect(renderToBufferMock).toHaveBeenCalled();
        });

        it("createHistogram calls render", async () => {
            await createHistogram([1], "L", "T", { min: 0, max: 10, binSize: 5 });
            expect(renderToBufferMock).toHaveBeenCalled();
        });
